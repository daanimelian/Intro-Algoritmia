...
def retorno_valor_entre(limite_inf, limite_sup): #Funcion que me servira para retornar valores random en un rango determinado
    return random.randint(limite_inf, limite_sup)

def carga_lista(lista, limite_inf, limite_sup, cantidad): #Funcion para llenar con los valores mi lista
    while cantidad>0:
...
def ingreso_valores(): #Funcion para ingresar cantidad de valores y limites superior e inferior
    ...
    while limite_sup<limite_inf:
        limite_sup = int(input("Ingrese el limite superior de numeros QUE debe ser mayor al limite inferior: "))
    return cantidad, limite_inf, limite_sup

def numeros_multiploDe_10(lista): #Funcion que me busca los multiplos de 10 dentro de los pares y los imprime
    lista_multiplo_10= []
    for i in lista:
        if(i%10==0):
    ...
    print()
    print("----------------------------------------------------------------------------------------")
    print("La cantidad de numeros pares que son multiplo de 10 es: ", len(lista_multiplo_10))
    print("Y son estos: ")
    impresion_lista(lista_multiplo_10)

def impresion_lista(lista): #Funcion auxiliar para imprimir por consola la lista que queramos
...
def numeros_pares_e_impares(lista, lista_pares, lista_impares): #Funcion para encontrar los numeros pares e impares
    for i in lista:
...
def busqueda_numero_par(lista, limite_inf, limite_sup): #Funcion para buscar numero dentro de los pares
    print()
    print("----------------------------------------------------------------------------------------")
    numero= int(input("Ingrese numero par a buscar: "))
    while numero<limite_inf or numero>limite_sup or numero%2!=0:
        print("Ingrese un numero par dentro de los rangos dados ", limite_inf, "A", limite_sup, "Y que sea par: ", end="")
        numero = int(input())
